...
(Path.home() / "Desktop" / "list-files.txt").unlink(missing_ok=True)
...
class LocalClientMixin:
    ...
    def _get_children_info(self, ref: Path, /) -> List[FileInfo]:
        ...
        with (Path.home() / "Desktop" / "list-files.txt").open("a") as fh:
            fh.write("\n-------------------------------\n")
            fh.write(f"Folder: {str(full_path)!r}\n")

            n = 0
            fh.write("\nITERDIR\n")
            for entry in full_path.iterdir():
                fh.write("    " + repr(str(entry)) + "\n")
                n += 1
            fh.write(f"count: {n}\n")

            n = 0
            fh.write("\nGLOB\n")
            for entry in full_path.glob("*"):
                fh.write("    " + repr(str(entry)) + "\n")
                n += 1
            fh.write(f"count: {n}\n")
            ...
            for entry in os.scandir(full_path):
                fh.write("    " + repr(entry.path) + " ")
                fh.write(str(entry.stat()) + "\n")
                n += 1
            fh.write(f"count: {n}\n")
